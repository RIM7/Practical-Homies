...
class Screw_Gauge_Main():
    def instruction_butt(self):
        startfile('..\\pdfs\\Screw Gauge.pdf')
        #webbrowser.open("https://www.youtube.com/watch?v=07d2dXHYb94", new=1)

    def apply_func(self, instr):        #global applied, x_left_lim, px, qx, rx, sx, tx, ux, grab_x
        self.r = self.ent.get()
        try:
            if float(self.r)>0.3 and instr=='insert': pass
            elif float(self.r)>=0 and float(self.r)<=0.3 and instr=='insert':
                self.applied = 1; 
                self.x_left_lim = 449.1+float(self.r)*10*10*2
                self.grab_x = 345; 
                self.px=720; self.qx=650; 
                self.rx=650; self.sx=720; 
                self.tx=980; self.ux=980;    #print(self.param[3])
                ...
                self.applied = 0; 
                self.x_left_lim = 449.1; 
                self.fast_left['state'] = 'normal'
                self.direction_button(''); 
    ...
    def direction_button(self, direction):
        ...
        if direction=='up': 
            self.first_thick_line -= 1; 
            self.second_thick_line -= 1; 
            self.third_thick_line -= 1; 
            
            self.right['state']='normal'
            
            if self.rx <= self.x_left_lim+1:  self.left['state']='disabled'
            if self.rx <= self.x_left_lim + 15: self.fast_left['state'] = 'disabled'    #px-=1; qx-=1; rx-=1; sx-=1; tx-=1; ux-=1; grab_x-=1
            ...
            if self.first_thick_line==-10:
                self.first_thick_line = self.second_thick_line; 
                self.second_thick_line = self.third_thick_line; 
                self.third_thick_line = 20
            ...
            if self.rx >= self.x_left_lim + 15: self.fast_left['state'] = 'normal'    #***************
            if self.rx >= self.x_right_lim: self.right['state']='disabled'

            self.px+=0.05; self.qx+=0.05; 
            self.rx+=0.05; self.sx+=0.05; 
            self.tx+=0.05; self.ux+=0.05; 
            self.grab_x+=0.05

            if self.third_thick_line==30:
                self.third_thick_line = self.second_thick_line; 
                self.second_thick_line = self.first_thick_line; 
                self.first_thick_line  = 0
        ...
        for i in range(21): 
            if i == self.first_thick_line:
                ...
                self.label1 = Label(self.canvas, text=str(self.temp.next.data), font=("Courier", 7, 'bold'), bg='NavajoWhite2')
                self.label1.pack(); 
                self.label1.place(x = self.qx+25, y=180+y_cir_inc)   
            elif i == self.second_thick_line:
                self.canvas.create_line(self.qx, 190+y_cir_inc, self.qx+20, 190+y_cir_inc, width=2)
                self.label2 = Label(self.canvas, text=str(self.temp.data), font=("Courier", 7, 'bold'), bg='NavajoWhite2')
                self.label2.pack(); self.label2.place(x = self.qx+25, y=180+y_cir_inc)   

            elif i == self.third_thick_line:
                self.canvas.create_line(self.qx, 190+y_cir_inc, self.qx+20, 190+y_cir_inc, width=2)
                self.label3 = Label(self.canvas, text=str(self.temp.prev.data), font=("Courier", 7, 'bold'), bg='NavajoWhite2')
    ...
    def faster_direction_button(self, direction):        #global rx, x_left_lim, temp #if rx >= x_right_lim: faster_right['state'] = 'disabled'
        if self.rx <= self.x_left_lim + 15: 
            self.fast_left['state'] = 'disabled'
        for i in range(150):
    ...
    def __init__(self, master):
        ...
        self.root.title('Screw Guage:'); self.root.resizable(0,0)
        self.root.iconbitmap('..\\img\\logos-and-icons\\AEC_logo.ico')

        self.canvas = Canvas(self.root,bg="NavajoWhite2"); self.canvas.pack(fill = 'both', expand=1); 
        def moved(event): self.canvas.itemconfigure(tag, text="(%r, %r)" % (event.x, event.y))
        self.canvas.bind("<Motion>", moved); tag = self.canvas.create_text(10, 10, text="", anchor="nw")
        
        self.x_left_lim  = 449.1
        self.x_right_lim = 650
        self.label1 = None
        self.label2 = None
        self.label3 = None
        self.applied = 0
        ...
        for i in range(21): 
            if i == self.first_thick_line:      
                self.canvas.create_line(self.qx, 190+y_cir_inc, self.qx+20, 190+y_cir_inc, width=2)
                self.label1 = Label(self.canvas, text=str(self.temp.next.data), font=("Courier", 7, 'bold'), bg='NavajoWhite2'); 
                self.label1.place(x =675, y=180+y_cir_inc)

            elif i == self.second_thick_line:      
                self.canvas.create_line(self.qx, 190+y_cir_inc, self.qx+20, 190+y_cir_inc, width=2)
                self.label2 = Label(self.canvas, text=str(self.temp.data), font=("Courier", 7, 'bold'), bg='NavajoWhite2'); 
                self.label2.place(x =675, y=180+y_cir_inc)

            elif i == self.third_thick_line:
                self.canvas.create_line(self.qx, 190+y_cir_inc, self.qx+20, 190+y_cir_inc, width=2)
                self.label3 = Label(self.canvas, text=str(self.temp.prev.data), font=("Courier", 7, 'bold'), bg='NavajoWhite2'); 
                self.label3.place(x =675, y=180+y_cir_inc)            
        ...
        for i in range(41):
            if i==0 or i==10 or i==20 or i==30 or i==40: 
                ...
                self.canvas.create_line(448+x_mm_inc, 230, 448+x_mm_inc, 220)
            else:        
                self.canvas.create_line(448+x_mm_inc, 230, 448+x_mm_inc, 240)
            x_mm_inc+=5

        self.left = Button(self.canvas,width=7,text="Left",bg='lemon chiffon',anchor = CENTER,font=("Courier",12,'bold'),command=lambda:self.direction_button('up')); 
        self.left.place(x=800, y= 30);
        self.right = Button(self.canvas,width=7,text="Right",bg='lemon chiffon',anchor=CENTER,font=("Courier",12,'bold'),command=lambda:self.direction_button('down')); 
        self.right.place(x=895, y=30);

        Label(self.canvas, text=' Enter the radius of the wire : \n\n(Enter a value between 0.0\n and 0.3)',justify=LEFT,font=("Courier", 12, 'bold'), bg='NavajoWhite2').place(x=235, y=35)
        self.ent=Entry(self.canvas, bd=4, width=6, relief=FLAT, font=('Courier',12,'bold')); self.ent.place(x=560, y=35)

        self.fast_left=Button(self.canvas,text="Move left Faster",bg='lemon chiffon',anchor=CENTER,font=("Courier",12,'bold'),command=lambda:self.faster_direction_button('up'));
        self.fast_left.place(x=800,y= 70)
        
        self.apply=Button(self.canvas,width=12,text="Insert Wire",bg='lemon chiffon',anchor=CENTER,font=("Courier",12,'bold'),command=lambda:self.apply_func('insert')); 
        self.apply.place(x=650, y= 30);
        
        self.remove=Button(self.canvas,width=12,text="Remove Wire",bg='lemon chiffon',anchor=CENTER,command=lambda:self.apply_func('remove'), font=("Courier", 12,'bold')); 
        self.remove.place(x=650,y=70);
        
        self.instruction=Button(self.canvas,width=12,text="Instruction",anchor=CENTER,bg='lemon chiffon',font=("Courier", 12, 'bold'),command=self.instruction_butt); 
        self.instruction.place(x=60,y=30);

        self.root.mainloop()

#screw_gauge_obj = Screw_Gauge(1, 0.2)