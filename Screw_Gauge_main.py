...
class Screw_Gauge_Main():
    ...
    def apply_func(self, instr):        #global applied, x_left_lim, px, qx, rx, sx, tx, ux, grab_x
        ...
        try:
            if float(self.r)>0.3 and instr=='insert': pass
                ...
                self.applied = 0; self.x_left_lim = 449.1; self.fast_left['state'] = 'normal'
                self.direction_button(''); 
        except: pass
    ...
    def faster_direction_button(self, direction):        #global rx, x_left_lim, temp #if rx >= x_right_lim: faster_right['state'] = 'disabled'
        if self.rx <= self.x_left_lim + 15: self.fast_left['state'] = 'disabled'
        for i in range(150):
    ...
    def __init__(self, master):
        ...
        self.label1 = None; self.label2 = None; self.label3 = None; self.applied = 0
        #dynamic part.
        ...
        for i in range(21): 
            if i == self.first_thick_line:      
                self.canvas.create_line(self.qx, 190+y_cir_inc, self.qx+20, 190+y_cir_inc, width=2)
                self.label1 = Label(self.canvas, text=str(self.temp.next.data), font=("Courier", 7, 'bold'), bg='NavajoWhite2'); self.label1.place(x =675, y=180+y_cir_inc)
            elif i == self.second_thick_line:      
                self.canvas.create_line(self.qx, 190+y_cir_inc, self.qx+20, 190+y_cir_inc, width=2)
                self.label2 = Label(self.canvas, text=str(self.temp.data), font=("Courier", 7, 'bold'), bg='NavajoWhite2'); self.label2.place(x =675, y=180+y_cir_inc)
            elif i == self.third_thick_line:
                self.canvas.create_line(self.qx, 190+y_cir_inc, self.qx+20, 190+y_cir_inc, width=2)
                self.label3 = Label(self.canvas, text=str(self.temp.prev.data), font=("Courier", 7, 'bold'), bg='NavajoWhite2'); self.label3.place(x =675, y=180+y_cir_inc)            
            else: self.canvas.create_line(self.qx, 190+y_cir_inc, self.qx+10, 190+y_cir_inc)
            y_cir_inc+=4
        ...
        self.left = Button(self.canvas,width=7,text="Left",bg='lemon chiffon',anchor = CENTER,font=("Courier",12,'bold'),command=lambda:self.direction_button('up')); self.left.place(x=800, y= 30);
        self.right = Button(self.canvas,width=7,text="Right",bg='lemon chiffon',anchor=CENTER,font=("Courier",12,'bold'),command=lambda:self.direction_button('down')); self.right.place(x=895, y=30);
        ...
        self.fast_left=Button(self.canvas,text="Move left Faster",bg='lemon chiffon',anchor=CENTER,font=("Courier",12,'bold'),command=lambda:self.faster_direction_button('up'));self.fast_left.place(x=800,y= 70)
        self.apply=Button(self.canvas,width=12,text="Insert Wire",bg='lemon chiffon',anchor=CENTER,font=("Courier",12,'bold'),command=lambda:self.apply_func('insert')); self.apply.place(x=650, y= 30);
        self.remove=Button(self.canvas,width=12,text="Remove Wire",bg='lemon chiffon',anchor=CENTER,command=lambda:self.apply_func('remove'), font=("Courier", 12,'bold')); self.remove.place(x=650,y=70);
        self.instruction=Button(self.canvas,width=12,text="Instruction",anchor=CENTER,bg='lemon chiffon',font=("Courier", 12, 'bold'),command=self.instruction_butt); self.instruction.place(x=60,y=30);
