...
class Screw_Gauge():
    ...
    def apply_func(self, instr):
        ...
        if float(argv[2])>0.3: pass
        else:
            if instr=='insert':
                self.applied = 1; self.x_left_lim = 449.1+float(argv[2])*10*10*2
                self.grab_x = 345; self.px=720; self.qx=650; self.rx=650; self.sx=720; self.tx=980; self.ux=980;
                print(argv[2])
                self.canvas.create_oval(144.1, 130, 144.1+float(argv[2])*10*10*2, 140); self.canvas.create_line(144.1, 135, 144.1, 305)
                self.canvas.create_line(144.1+float(argv[2])*10*10*2, 135, 144.1+float(argv[2])*10*10*2, 305); 
                self.canvas.create_oval(144.1, 300, 144.1+float(argv[2])*10*10*2, 310)
    ...
    def direction_button(self, direction):
        ...
        if self.applied==1: 
            self.canvas.create_oval(144.1, 130, 144.1+float(argv[2])*10*10*2, 140); 
    ...
    def __init__(self):
        ...
        self.root.title("Screw Guage: Measurement number: " + argv[1]); self.root.resizable(0,0)
        ...
        #faster_right = Button(canvas, text = "Move left Faster", anchor = CENTER, command = lambda: faster_direction_button('down'))
        #faster_right.pack(); faster_right.place(x=800, y= 480); faster_right.config(font=("Courier", 12, 'bold'))
