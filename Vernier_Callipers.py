...
class Vernier_Callipers():
	...
	def direction_button(self,direction):
		...
		if self.which_opt == 'width':
			self.canvas.create_oval(145, 245, 145+self.dia, 275);
			self.canvas.create_line(145, 260, 145, 360); self.canvas.create_line(145+self.dia, 260, 145+self.dia, 360);
			self.canvas.create_oval(145, 345, 145+self.dia, 375)
			self.x_stop_left = 145+ self.dia
		elif self.which_opt == 'height':
			self.canvas.create_oval(145-15, 310-40, 145+15, 310+40)
			self.canvas.create_line(145, 270, 145+self.hig, 270); self.canvas.create_line(145, 350, 145+self.hig, 350); 
			self.canvas.create_oval(145+self.hig-15, 310-40, 145+self.hig+15, 310+40)
	    ...
	    #static part
		self.canvas.create_arc(70, 50, 140, 190, start = 0)
		self.canvas.create_line(self.px1,self.py1, self.px2,self.py2,self.px3,self.py3,self.px4,self.py4,self.px5,self.py5,self.px6,self.py6,self.px7,self.py7,self.px8,self.py8,self.px4,self.py4,self.px9,self.py9,self.px1,self.py1,width=1.4)
		self.x_cm_inc = 0
		for i in range(60):
			if i%10==0:             self.canvas.create_line(200 + self.x_cm_inc, 140, 200 + self.x_cm_inc, 120); self.x_cm_inc += 12.7
			elif i%10 in (2,4,6,8): self.canvas.create_line(200 + self.x_cm_inc, 130, 200 + self.x_cm_inc, 120); self.x_cm_inc += 12.7
			else:                   self.canvas.create_line(200 + self.x_cm_inc, 125, 200 + self.x_cm_inc, 120); self.x_cm_inc += 12.7
		self.x_cm_inc = 0
		for i in range(151):
			if i%10==0:   self.canvas.create_line(200 + self.x_cm_inc, 210, 200 + self.x_cm_inc, 185); self.x_cm_inc += 5
			elif i%10==5: self.canvas.create_line(200 + self.x_cm_inc, 210, 200 + self.x_cm_inc, 190); self.x_cm_inc += 5
			else:         self.canvas.create_line(200 + self.x_cm_inc, 210, 200 + self.x_cm_inc, 195); self.x_cm_inc += 5

		#dynamic part-arc
		self.canvas.create_arc(self.arc_x0, self.arc_y0, self.arc_x1, self.arc_y1, start = 90)
		#dynamic part-lines
		self.canvas.create_line(self.ax,self.ay,self.bx,self.by,self.cx,self.cy,self.dx,self.dy,self.ax,self.ay,self.ex,self.ey,self.fx,self.fy,self.gx,self.gy,self.hx,self.hy,self.ix,self.iy,self.ex,self.ey,self.jx,self.jy,self.cx,self.cy, width=1.4)
		#dynamic part-scale body
		self.canvas.create_line(self.useless_ax,self.useless_ay,self.useless_cx,self.useless_cy,self.useless_dx,self.useless_dy,self.useless_bx,self.useless_by,width=1.4)
		#dynamic part-scale body
		for i in range(11): self.canvas.create_line(self.x1_vern + self.x_vern_inc, 230, self.x2_vern + self.x_vern_inc, 210); self.x_vern_inc+=4.5
	...
	def apply(self):
		...
		if self.which_opt == 'width':
			self.canvas.create_oval(145, 245, 145+self.dia, 275); self.canvas.create_line(145, 260, 145, 360)
			self.canvas.create_line(145+self.dia, 260, 145+self.dia, 360); self.canvas.create_oval(145, 345, 145+self.dia, 375)
			self.x_stop_left = 145+ self.dia
		elif self.which_opt == 'height':
			self.canvas.create_oval(145-15, 310-40, 145+15, 310+40)
			self.canvas.create_line(145, 270, 145+self.hig, 270)
			self.canvas.create_line(145, 350, 145+self.hig, 350); 
			self.canvas.create_oval(145+self.hig-15, 310-40, 145+self.hig+15, 310+40)
			self.x_stop_left = 145+ self.hig
	...
	def __init__(self, setup_val, R, h):
		...
		self.root = Tk();self.root.geometry('1000x600'); self.root.resizable(0,0); self.root.iconbitmap('AEC_logo.ico')
		try: self.root.title('Vernier Callipers: Measurement number: ' + str(setup_val))
		except: self.root.title('Vernier Callipers')

		self.canvas1 = Canvas(self.root, bg='peach puff3', height=100); self.canvas1.pack(side='top', fill = 'both')
		...
		self.canvas = Canvas(self.root, bg='white', height=500); self.canvas.pack(side='bottom', fill = 'both', expand=1)
		def moved(event): self.canvas.itemconfigure(tag, text="(%r, %r)" % (event.x, event.y))
		self.canvas.bind("<Motion>", moved); tag = self.canvas.create_text(10, 10, text="", anchor="nw")
		...
		#====================================================================================================
		self.canvas.create_arc(70, 50, 140, 190, start = 0)
		self.canvas.create_arc(70, 50, 140, 190, start = 90)
		...
		for i in range(151):
			if i%10==0:
				self.canvas.create_line(200 + self.x_cm_inc, 210, 200 + self.x_cm_inc, 185);
				self.label = Label(self.canvas, text=str(i//10), font=("Courier", 10, 'bold'), bg='white'); 
		...
		self.left=Button(self.canvas,width=5,text="Left",anchor=CENTER,command=lambda:self.direction_button('left'))
		self.left.pack(); self.left.place(x=800, y= 400); self.left.config(font=("Courier", 12, 'bold')); self.left['state'] = 'disabled'

		self.right=Button(self.canvas,width=5,text="Right",anchor=CENTER,command=lambda:self.direction_button('right'))
		self.right.pack(); self.right.place(x=900, y= 400); self.right.config(font=("Courier", 12, 'bold'))


	    #=====================================================================================================================
		self.var=IntVar()
		self.measure1=Radiobutton(self.canvas1,text='Measure by width',width=18,bg='peach puff3',variable=self.var,value=1,command=self.m)
		self.measure1.config(font=("Courier", 12, 'bold')); self.measure1.pack(padx=20, pady=20); self.measure1.place(x=250, y=20);

		self.measure2=Radiobutton(self.canvas1,text='Measure by height',width=18,bg='peach puff3',variable=self.var,value=2,command=self.m); 
		self.measure2.config(font=("Courier", 12, 'bold')); self.measure2.pack(padx=20, pady=20); self.measure2.place(x=254, y=60);

		self.apply=Button(self.canvas1,text="Use shape",anchor=CENTER,bg='peach puff3',command=self.apply)
		self.apply.pack(); self.apply.place(x=480, y=60); self.apply.config(font=("Courier", 12, 'bold'))

		self.instr_butt = Button(self.canvas1, text="Instructions", bg='peach puff3', command=self.instr_window)
		self.instr_butt.config(font=("Courier", 12, 'bold')); self.instr_butt.pack(); self.instr_butt.place(x=70, y=20)

		self.root.mainloop()


#vernier_callipers = Vernier_Callipers(master, 1, 3.4, 4)
vernier_callipers = Vernier_Callipers(sys.argv[1], sys.argv[2], sys.argv[2])