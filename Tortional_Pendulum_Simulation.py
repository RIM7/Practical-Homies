...
class Tortional_Pendulum():
    ...
    class StopWatch(Frame):  
        ...
        def Stop(self):
            if self.onRunning:
                ...
                self.onRunning = 0
        def Exit(self):  root.destroy(); exit()
    ...
    def commit(self):
        try: self.setup_val = int(self.setup.get()); self.ok_button['state']='normal'
        except: pass 
    ...
    def ok_clicked(self):
        ...
        try:
            ...
            popup = Tk(); popup.wm_title("Note"); popup.configure(background='lemon chiffon')
            ...
            B1.pack(); 
            popup.mainloop()    #return 0
        #=============================================================================
        #print('\nsetup_val = ', self.setup_val)
        try:
        ...
        system('python Vernier_Callipers.py '+str(self.setup_val)+' '+str(self.R)+' '+str(4));
        system('python Screw_Gauge.py '+str(self.setup_val)+' '+str(self.r)+' ') 
        #vernier_callipers = Vernier_Callipers(self.root, self.setup_val, self.R, 4)
    ...
    def __init__(self,master):   #=None
        ...
        #yt link button.
        #self.youtube_link=Button(self.canvas1,text="Click here to go to the tutorial", bg='peach puff3', command=self.YT_Click)
        #self.youtube_link.config(font=("Courier", 12, 'bold')); self.youtube_link.pack(); self.youtube_link.place(x=30, y=30)
        ...
        self.instr_tortional_butt.config(font=("Courier", 12, 'bold')); self.instr_tortional_butt.pack(); self.instr_tortional_butt.place(x=30, y=70)
        
        self.canvas1.create_line(0, 115, 400, 115, width=2)

        self.valid=Label(self.canvas1,text='Enter the validity of the setup\n(in iterations):                ',justify=LEFT,bg='peach puff3') 
        self.valid.config(font=("Courier", 12, 'bold')); self.valid.pack(); self.valid.place(x=30, y=130)

        self.setup = Entry(self.canvas1, relief=tk.FLAT, width = 20); 
        self.setup.pack(); self.setup.place(x=30, y=200); self.setup.config(font=("Courier", 12, 'bold'))

        self.commit = Button(self.canvas1, text="Commit", bg='peach puff3', command=self.commit)
        self.commit.config(font=("Courier", 12, 'bold')); self.commit.pack(); self.commit.place(x=270, y=195)

        self.osc_label=Label(self.canvas1,text='Enter the no. of oscillations:',justify=LEFT, bg='peach puff3',fg='black'); 
        self.osc_label.config(font=("Courier", 12, 'bold')); self.osc_label.pack(); self.osc_label.place(x=30, y=240)

        self.osc = Entry(self.canvas1, relief=tk.FLAT, width = 20); 
        self.osc.pack(); self.osc.place(x=30, y=280);
        self.osc.config(font=("Courier", 12, 'bold'))


        self.ok_button = Button(self.canvas1, width=5, text="ok", bg='peach puff3', anchor=CENTER, command=self.ok_clicked)
        self.ok_button.pack(padx=20, pady = 20);self.ok_button.place(x=270, y= 275);
        self.ok_button.config(font=("Courier", 12, 'bold'))
        self.ok_button['state']='disabled'
        #submit button.
        #self.submit_tort_butt=Button(self.canvas1,text="Submit",font=("Courier",12,'bold'),bg='peach puff3',command=self.create_submission_window)
        #self.submit_tort_butt.pack();self.submit_tort_butt.place(x=270,y=427);#self.submit_tort_butt['state']='disabled';
        ...
        #self.M_value=Label(self.canvas1,text='',justify='left',font=('Courier',12,'bold'),bg='peach puff3'); self.M_value.place(x=80,y=490)
    ...
    #===========================================================================================
    @classmethod
    def draw_on_next_canvas(cls,canvas):
        #global setup_val;global L,r,M,R,ita,theta;L=float(entries[0].get());r=float(entries[1].get());R=float(entries[2].get())#M=float(entries[3].get());ita=8.9*10**11;theta=10;
        try: 
            ...
            while loop>0:
                ...
                for i in range(len_x):
                    try:
                        Tortional_Pendulum.top_view = canvas.create_oval((start_of_string_x-rad), (start_of_string_y-rad), 
